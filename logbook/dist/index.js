...
let RoutesViewerElement = class extends UkcLogbookElement$1 {
  ...
  get routeKeyForSortOrder() {
    return this.sort_order.value === "ltr" ? "buttress_name" : "route_grade";
  }
  async reorderBaseData() {
    ...
    if (!data)
      return;
    let objects = [];
    const routes = await this.getRoutesFromIndex("");
    let currentSection = { title: "", ids: [], routes: [] };
    const key = this.routeKeyForSortOrder;
    routes.forEach((route) => {
      if (currentSection.title !== route[key]) {
        currentSection = { title: route[key], ids: [], routes: [] };
  ...
  async performInitialFetch() {
    ...
    if (this.isOnline) {
      ...
      dataJson.objects = this._buildIdsSectionArray(dataJson);
      ...
      void this._buildInvertedIndex(dataJson);
    }
  }
  async maybeBuildFromLocalStorage() {
    if (this.baseData) {
      await this._buildInvertedIndex(this.baseData);
      this.buildResults(this.baseData);
      this.stopProgressBar();
    }
  }
  async fetchResultsForPage(query, pageNo) {
    if (this.isOnline) {
      ...
      console.log("offline");
      return this.fetchResultsForPageLocal(query);
    }
  }
  ...
  async getRoutesFromIndex(query) {
    this.index || await this._buildInvertedIndex(this.baseData);
    if (!query) {
      const routes = [];
  ...
  async fetchResultsForPageLocal(query) {
    ...
    routes.forEach((route) => {
      count++;
      if (currentSection.title !== route[key]) {
        currentSection = { title: route[key], ids: [] };
        objects.push(currentSection);
      }
      currentSection.ids.push(route.id_ukc);
    });
  ...
  async _buildInvertedIndex(data) {
    const addFun = (route) => {
